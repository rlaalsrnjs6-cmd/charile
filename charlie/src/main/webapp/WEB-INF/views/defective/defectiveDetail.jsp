...
<div class="mat-all">
    ...
    <div class="dt-wrap">
        <span class="lb0">불량품 상세정보</span> 

        <div class="dt-row">
            <span class="dt-lb">불량번호</span> 
            <span class="dt-val">${map.list[0].defective_num}</span>
        </div>
        
        <div class="dt-row">
            <span class="dt-lb">불량카테고리</span> 
            <span class="dt-val">${map.list[0].category}</span>
        </div>
        
        <div class="dt-row">
            <span class="dt-lb">개수</span> 
            <span class="dt-val">${map.list[0].count}</span>
        </div>
        
        <div class="dt-row">
            <span class="dt-lb">불량조치방법</span> 
            <span class="dt-val">${map.list[0].action}</span>
        </div>
        
        <div class="dt-row">
            <span class="dt-lb">로트번호</span> 
            <span class="dt-val">${map.list[0].lot_num}</span>
        </div>

        <div class="btn-grp">
            <a href="defective?mod=list" class="btn-list">목록으로</a>
            <a href="defective?defective_num=${map.list[0].defective_num}&mod=up" class="btn-up">수정</a>
            <a href="defective?defective_num=${map.list[0].defective_num}&mod=delete" class="btn-del">삭제</a>
        </div>
    </div>
...
<script>
    ...
    if (btnDel) {
        btnDel.addEventListener('click', (e) => {
            if(!confirm("정말로 해당 불량품 정보를 삭제하시겠습니까?")) {
                e.preventDefault();
            }
        });
    }
