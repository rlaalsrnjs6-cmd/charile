...
<%-- 		<a href="mdm2?cmd=list&page=<%= start_section-1 %>size=<%= size %>"> --%>
	...
	<c:forEach var="i" begin="<%= start_section %>" end="<%= end_section %>">
		<c:if test="${ pageNum eq i }">
			<a class="pg-btn pg-active" href="${servletName}?cmd=list&page=${i}&size=<%= size %>&mod=select&category=${category}D">
				<strong>${i}</strong>
			</a>
		</c:if>
		
		<c:if test="${ pageNum ne i }">
			<a class="pg-btn" href="javascript:void(0);" onclick="loadPage(${i})">
				${i}
			</a>
		</c:if>
	</c:forEach>
